        ...
        struct sr_mod_info_s *mod_info)
{
    ...
    if (prev_mod_type < MOD_INFO_INV_DEP) {
        ...
        shm_deps = (sr_mod_dep_t *)(mod_info->conn->main_shm.addr + shm_mod->deps);
    ...
    if (prev_mod_type < MOD_INFO_REQ) {
        ...
        while ((dep_mod = sr_shmmain_getnext(mod_info->conn->main_shm.addr, dep_mod))) {
            shm_deps = (sr_mod_dep_t *)(mod_info->conn->main_shm.addr + dep_mod->deps);
...
sr_shmmod_multilock(struct sr_mod_info_s *mod_info, int wr, int applying_changes)
{
    ...
    for (i = 0; i < mod_info->mod_count; ++i) {
        ...
        while (steps) {
            ...
            if (!applying_changes || !mod_info->mods[i].shm_mod->conf_sub[mod_info->ds].applying_changes) {
                break;
            }
        ...
        if (applying_changes && mod_wr) {
            ...
            mod_info->mods[i].shm_mod->conf_sub[mod_info->ds].applying_changes = 1;
...
sr_shmmod_multirelock(struct sr_mod_info_s *mod_info, int upgrade)
{
    ...
    for (i = 0; i < mod_info->mod_count; ++i) {
        if ((mod_info->mods[i].state & (MOD_INFO_REQ | MOD_INFO_LOCK)) == (MOD_INFO_REQ | MOD_INFO_LOCK)) {
            ...
            SR_CHECK_INT_RET(!mod_info->mods[i].shm_mod->conf_sub[mod_info->ds].applying_changes, err_info);
    ...
    for (i = 0; i < mod_info->mod_count; ++i) {
        if (mod_info->mods[i].state & MOD_INFO_LOCK) {
            if ((mod_info->mods[i].state & MOD_INFO_REQ) && applying_changes) {
                assert(mod_info->mods[i].shm_mod->conf_sub[mod_info->ds].applying_changes);
                mod_info->mods[i].shm_mod->conf_sub[mod_info->ds].applying_changes = 0;
            }
            sr_shmmod_unlock(mod_info->mods[i].shm_mod, mod_info->ds);
        }
    }
...
static sr_error_info_t *
sr_shmmod_module_config_data_get(struct ly_ctx *ly_ctx, char *shm, sr_mod_t *shm_mod, sr_datastore_t ds, struct lyd_node **data)
{
    sr_error_info_t *err_info = NULL;
    ...
    *data = NULL;
    ...
    if (asprintf(&path, "%s/data/%s.%s", sr_get_repo_path(), shm + shm_mod->name, sr_ds2str(file_ds)) == -1) {
    ...
    *data = lyd_parse_path(ly_ctx, path, LYD_LYB, LYD_OPT_CONFIG | LYD_OPT_NOEXTDEPS);
    ...
    case SR_DS_OPERATIONAL:
        ...
        xpaths = NULL;
        shm_confsubs = (sr_mod_conf_sub_t *)(shm + shm_mod->conf_sub[SR_DS_RUNNING].subs);
        for (i = 0, xp_i = 0; i < shm_mod->conf_sub[SR_DS_RUNNING].sub_count; ++i) {
            if (shm_confsubs[i].xpath) {
                xpaths = sr_realloc(xpaths, (xp_i + 1) * sizeof *xpaths);
                SR_CHECK_MEM_GOTO(!xpaths, err_info, error);

                xpaths[xp_i] = shm + shm_confsubs[i].xpath;
                ++xp_i;
            }
        }

        /* filter out disabled subtrees */
        err_info = sr_ly_data_dup_filter(*data, xpaths, xp_i, &tmp);
        free(xpaths);
        if (err_info) {
            goto error;
        }
...
sr_shmmod_data_update(struct sr_mod_info_s *mod_info, uint8_t mod_type, int state_data, sr_error_info_t **cb_error_info)
{
    ...
    for (i = 0; i < mod_info->mod_count; ++i) {
        ...
        if (mod->state & mod_type) {
            ...
            if ((err_info = sr_shmmod_module_config_data_get(mod->ly_mod->ctx, mod_info->conn->main_shm.addr, mod->shm_mod,
...
static sr_error_info_t *
sr_shmmod_module_config_data_set(const char *mod_name, sr_datastore_t ds, struct lyd_node *data)
{
    char *path;
    sr_error_info_t *err_info = NULL;
    ...
    *result = NULL;
    ...
    for (i = 0; i < mod_info->mod_count; ++i) {
        ...
        if (mod->state & MOD_INFO_REQ) {
            ...
            case SR_EV_NONE:
                /* edit */
                err_info = sr_ly_edit_mod_apply(session->dt[session->ds].edit, mod, NULL);
                break;
            case SR_EV_UPDATE:
                /* both */
                err_info = sr_ly_diff_mod_apply(session->dt[session->ds].diff, mod);
                if (!err_info) {
                    err_info = sr_ly_edit_mod_apply(session->dt[session->ds].edit, mod, NULL);
                }
                break;
...
sr_modinfo_add_instid_deps(sr_conn_ctx_t *conn, sr_mod_t *shm_mod, const struct lyd_node *mod_data, struct ly_set *dep_set)
{
    ...
    shm_deps = (sr_mod_dep_t *)(conn->main_shm.addr + shm_mod->deps);
    for (i = 0; i < shm_mod->dep_count; ++i) {
        if (shm_deps[i].type == SR_DEP_INSTID) {
            if (mod_data) {
                set = lyd_find_path(mod_data, conn->main_shm.addr + shm_deps[i].xpath);
            } else {
...
sr_shmmod_validate(struct sr_mod_info_s *mod_info, int finish_diff)
{
    ...
    for (i = 0; i < dep_set->number; ++i) {
        ...
        SR_CHECK_INT_GOTO(!shm_mod, err_info, cleanup);
        if ((err_info = sr_shmmod_module_config_data_get(mod_info->conn->ly_ctx, mod_info->conn->main_shm.addr, shm_mod,
...
sr_shmmod_store(struct sr_mod_info_s *mod_info)
{
    ...
    for (i = 0; i < mod_info->mod_count; ++i) {
        ...
        if (mod->state & MOD_INFO_CHANGED) {
            /* set the new data */
            if ((err_info = sr_shmmod_module_config_data_set(mod->ly_mod->name, mod_info->ds, mod->mod_data))) {
                return err_info;
            }
        }
    }
        ...
        uint32_t priority, int subscr_opts, int add)
{
    ...
    assert(!(subscr_opts & ~SR_SUBSCR_UPDATE));
    ...
    if (add) {
        ...
        for (i = 0; i < shm_mod->conf_sub[ds].sub_count; ++i) {
            if ((!xpath && !shm_sub[i].xpath)
                ...
                if ((shm_sub[i].priority == priority) && (shm_sub[i].opts == subscr_opts)) {
                    break;
                }
            }
        }
        /*if (i == shm_mod->conf_sub[ds].sub_count) {
            SR_ERRINFO_INT(&err_info);
            sleep(40);
            return err_info;
        }*/
        ...
        if (i < shm_mod->conf_sub[ds].sub_count - 1) {
            memcpy(&shm_sub[i], &shm_sub[shm_mod->conf_sub[ds].sub_count - 1], sizeof *shm_sub);
        }
