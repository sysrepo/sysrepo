...
sr_shmsub_notify_write_event(sr_sub_shm_t *sub_shm, sr_cid_t orig_cid, uint32_t request_id, sr_sub_event_t event,
        struct sr_sid_s *sid, sr_shm_t *shm_data_sub, const char *xpath, const char *data, uint32_t data_len,
...
sr_shmsub_multi_notify_write_event(sr_multi_sub_shm_t *multi_sub_shm, sr_cid_t orig_cid, uint32_t request_id,
        uint32_t priority, sr_sub_event_t event, struct sr_sid_s *sid, uint32_t subscriber_count, sr_shm_t *shm_data_sub,
        time_t notif_ts, const char *data, uint32_t data_len, const char *event_desc)
{
    sr_error_info_t *err_info = NULL;
    const char *user = NULL;
    char *shm_data_ptr;
    ...
    *update_edit = NULL;
    ...
    while ((mod = sr_modinfo_next_mod(mod, mod_info, mod_info->diff, &aux))) {
        ...
        do {
            ...
            /* parse updated edit */
            ly_errno = 0;
            edit = lyd_parse_mem(ly_ctx, shm_data_sub.addr, LYD_LYB, LYD_OPT_EDIT | LYD_OPT_STRICT);
            if (ly_errno) {
                sr_errinfo_new_ly(&err_info, ly_ctx);
    ...
    while ((mod = sr_modinfo_next_mod(mod, mod_info, mod_info->diff, &aux))) {
        ...
        if (multi_sub_shm->event == SR_SUB_EV_ERROR) {
            ...
            if ((err_info = sr_shmsub_multi_notify_write_event(multi_sub_shm, 0, mod->request_id,
                    multi_sub_shm->priority, 0, NULL, 0, NULL, 0, NULL, 0, NULL))) {
                goto cleanup_wrunlock;
            }
        ...
        struct lyd_node **data, sr_error_info_t **cb_err_info)
{
    ...
    if ((err_info = sr_shmsub_notify_write_event(sub_shm, cid, request_id, SR_SUB_EV_OPER, &sid, &shm_data_sub,
    ...
    /* parse returned data */
    ly_errno = 0;
    *data = lyd_parse_mem(ly_mod->ctx, shm_data_sub.addr, LYD_LYB, LYD_OPT_DATA | LYD_OPT_TRUSTED | LYD_OPT_STRICT);
    if (ly_errno) {
        sr_errinfo_new_ly(&err_info, ly_mod->ctx);
        ...
        sr_sid_t sid, uint32_t timeout_ms, uint32_t *request_id, struct lyd_node **output, sr_error_info_t **cb_err_info)
{
    ...
    *output = NULL;
    ...
    /* open sub data SHM */
    if ((err_info = sr_shmsub_data_open_remap(lyd_node_module(input)->name, "rpc", sr_str_hash(op_path), &shm_data_sub, 0))) {
        goto cleanup_wrunlock;
    }
    ...
    do {
        ...
        if ((err_info = sr_shmsub_multi_notify_write_event(multi_sub_shm, conn->cid, *request_id, cur_priority,
                SR_SUB_EV_RPC, &sid, subscriber_count, &shm_data_sub, 0, input_lyb, input_lyb_len, op_path))) {
            goto cleanup_wrunlock;
        }
        ...
        ly_errno = 0;
        *output = lyd_parse_mem(lyd_node_module(input)->ctx, shm_data_sub.addr, LYD_LYB,
...
clear_shm:
        ...
        if ((err_info = sr_shmsub_multi_notify_write_event(multi_sub_shm, 0, request_id, cur_priority, 0, NULL, 0,
    ...
    do {
        ...
        if ((err_info = sr_shmsub_multi_notify_write_event(multi_sub_shm, conn->cid, request_id, cur_priority,
                SR_SUB_EV_ABORT, &sid, subscriber_count, &shm_data_sub, 0, input_lyb, input_lyb_len, op_path))) {
            goto cleanup_wrunlock;
        }
...
sr_shmsub_notif_notify(sr_conn_ctx_t *conn, const struct lyd_node *notif, time_t notif_ts, sr_sid_t sid)
{
    ...
    if ((err_info = sr_shmsub_multi_notify_write_event(multi_sub_shm, conn->cid, request_id, 0, SR_SUB_EV_NOTIF, &sid,
...
sr_shmsub_change_listen_process_module_events(struct modsub_change_s *change_subs, sr_conn_ctx_t *conn)
{
    ...
    /* parse event diff */
    diff = lyd_parse_mem(conn->ly_ctx, shm_data_ptr, LYD_LYB, LYD_OPT_EDIT | LYD_OPT_STRICT);
...
process_event:
    ...
    case SR_SUB_EV_UPDATE:
        if (err_code == SR_ERR_OK) {
            ...
            LY_TREE_FOR(ev_sess->dt[ev_sess->ds].edit, iter) {
            ...
            /* print it into LYB */
            if (lyd_print_mem(&data, ev_sess->dt[ev_sess->ds].edit, LYD_LYB, LYP_WITHSIBLINGS)) {
...
sr_shmsub_oper_listen_process_module_events(struct modsub_oper_s *oper_subs, sr_conn_ctx_t *conn)
{
    ...
    for (i = 0; (err_code == SR_ERR_OK) && (i < oper_subs->sub_count); ++i) {
        ...
        /* parse data parent */
        ly_errno = 0;
        parent = lyd_parse_mem(conn->ly_ctx, shm_data_ptr, LYD_LYB, LYD_OPT_DATA | LYD_OPT_STRICT | LYD_OPT_TRUSTED);
        if (ly_errno) {
            sr_errinfo_new_ly(&err_info, conn->ly_ctx);
            SR_ERRINFO_INT(&err_info);
            goto error_rdunlock;
    ...
    for (i = 0; i < rpc_subs->sub_count; ++i) {
        ...
        if (sr_shmsub_rpc_listen_is_new_event(multi_sub_shm, rpc_sub)) {
            ...
            if (!ev_sess) {
                ...
                ly_errno = 0;
                input = lyd_parse_mem(conn->ly_ctx, shm_data_ptr, LYD_LYB, LYD_OPT_RPC | LYD_OPT_STRICT | LYD_OPT_TRUSTED,
                        NULL);
                SR_CHECK_LY_GOTO(ly_errno, conn->ly_ctx, err_info, cleanup_rdunlock);
            }
            assert(input);
...
sr_shmsub_notif_listen_process_module_events(struct modsub_notif_s *notif_subs, sr_conn_ctx_t *conn)
{
    ...
    /* parse notification */
    ly_errno = 0;
    notif = lyd_parse_mem(conn->ly_ctx, shm_data_ptr, LYD_LYB, LYD_OPT_NOTIF | LYD_OPT_STRICT | LYD_OPT_TRUSTED, NULL);
