 ...
 */
static sr_error_info_t *
sr_lydmods_load_module(const struct lyd_node *sr_mod, struct ly_ctx *ly_ctx, const struct lys_module **ly_mod_p)
{
    sr_error_info_t *err_info = NULL;
    struct lyd_node *node;
    const struct lys_module *ly_mod;
...
sr_lydmods_sched_ctx_load_modules(const struct lyd_node *sr_mods, struct ly_ctx *ly_ctx, int removed, int updated, int *change)
{
    ...
    LY_TREE_FOR(sr_mods->child, sr_mod) {
        ...
        /* load the module */
        if ((err_info = sr_lydmods_load_module(sr_mod, ly_ctx, NULL))) {
            return err_info;
        }
    }
...
sr_lydmods_sched_update_data(const struct lyd_node *sr_mods, const struct ly_ctx *new_ctx, int *fail)
{
    ...
    ly_errno = 0;
    new_data = lyd_parse_mem((struct ly_ctx *)new_ctx, data_json, LYD_JSON, LYD_OPT_CONFIG);
    if (ly_errno) {
...
sr_lydmods_deferred_add_module(struct ly_ctx *ly_ctx, const struct lys_module *ly_mod, const char **features, int feat_count)
{
    ...
    if (asprintf(&path, "/" SR_YANG_MOD ":sysrepo-modules/installed-module[name=\"%s\"]", ly_mod->name) == -1) {
        SR_ERRINFO_MEM(&err_info);
        goto cleanup;
    }
