        ...
        struct lyd_node **data)
{
    ...
    if ((err_info = sr_xpath_oper_data_get(ly_mod, sub_xpath, request_xpath, sid, shm_msub->evpipe_num,
            oper_parent, timeout_ms, cid, &oper_data))) {
        return err_info;
    }
        ...
        uint32_t timeout_ms, sr_get_oper_options_t opts)
{
    ...
    for (i = 0; i < mod_info->mod_count; ++i) {
        ...
        if ((err_info = sr_modinfo_module_data_load(mod_info, mod, sid, request_xpath, timeout_ms, opts))) {
